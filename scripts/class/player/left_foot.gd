...
func _stated(pose: Player.POSE) -> void:
	...
	match pose:
		...
		Player.POSE.WALK:
			...
			_tween["pos"].tween_property(self, "position", _init_state["pos"] + Vector2.UP * 2, 0.1)
			_tween["pos"].chain().tween_property(self, "position", _init_state["pos"], 0.1)
			_tween["pos"].chain().tween_property(self, "position", _init_state["pos"] + Vector2.LEFT * 2, 0.1)
		Player.POSE.JUMP:
			_anim_init("pos")
